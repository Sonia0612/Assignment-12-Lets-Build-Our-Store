    ...
    <div className='flex'>
      ...
      <div className='m-5 flex flex-wrap'>
        ...
        <ul className='menu-items'>
          {(menu?.cards[3]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.itemCards).map((me)=>(<li 
          key={me?.card?.info?.id}>{me?.card?.info?.name}
          <button className="bg-green-100 p-2 m-2" onClick={()=>addFoodItem(me?.card?.info)}>Add Item</button>
